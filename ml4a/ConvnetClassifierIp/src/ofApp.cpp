...
void ofApp::setup() {
    ...
    // test ip: http://195.200.199.8/axis-cgi/mjpg/video.cgi?resolution=320x240
    ipadress = "osp.local:8080/?action=stream";
    
    //IP GRABBER
    auto grabber = std::make_shared<ofx::Video::IPVideoGrabber>();
    grabber->setURI(ipadress);
    grabber->connect();
    grabbers = grabber;
    ...
    ccv.setup(ofToDataPath("../../../../models/image-net-2012.sqlite3"));
    ...
    receiver.setup(DEFAULT_OSC_LISTEN);
...
void ofApp::update() {
    ...
    grabbers->update(); // IP GRABBER
    if(grabbers->need_to_reconnect) {
        
        ofLogError("reconnecting...");
    ...
    featureEncoding = ccv.encode(grabbers->getPixels(), ccv.numLayers()-1);
...
void ofApp::draw() {
    ...
    ofBackground(150);
    ofSetColor(255);
    grabbers->draw(270, 200); // IP GRABBER
